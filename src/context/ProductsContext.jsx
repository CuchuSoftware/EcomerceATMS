...
export const ProductsProvider = ({ children }) => {
  ...
  useEffect(() => {
    const cargarProductos = async () => {
      try {
        const respuesta = await fetch('https://693f414212c964ee6b6f800c.mockapi.io/api/atms/ModeloATM');
  ...
  const editarProducto = async (productoActualizado) => {
    try {
      const respuesta = await fetch(`https://693f414212c964ee6b6f800c.mockapi.io/api/atms/ModeloATM/${productoActualizado.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
